...
```html
<p style="color: var(--muted-foreground);">Secondary information</p>
```
...
```html
<div
  style="background-color: var(--popover); color: var(--popover-foreground);"
>
  Popover content
</div>
```
...
```html
<div style="border-radius: var(--radius);">Rounded corners</div>
```
...
<button
  style="background-color: var(--warning); color: var(--warning-foreground);"
>
  Caution
</button>
```
...
export function renderCard(node: AstNode): string {
  ...
  const styles =
    'background-color: var(--card); border-color: var(--border); color: var(--card-foreground);'
...
2. **Use Tailwind for structure/spacing only**:
   ...
   <div class="flex items-center gap-4">
     <!-- ❌ Wrong -->
     <div class="flex items-center gap-4 bg-gray-800"></div>
   </div>
   ```
