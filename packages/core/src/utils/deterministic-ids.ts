 ...
 */
function extractStableProps(props: Record<string, unknown>): string {
  if (!props) return ''
 ...
 */
export function generateDeterministicIds(
  ast: AstNode | AstNode[],
  previousAst?: AstNode | AstNode[]
): AstNode | AstNode[] {
  // Handle array case
  if (Array.isArray(ast)) {
    ...
    for (let i = 0; i < astCopy.length; i++) {
      ...
      const previousNode =
        Array.isArray(previousAst) && previousAst[i]
          ? previousAst[i]
          : undefined
