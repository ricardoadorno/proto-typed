...
export const elementStyles = {
  ...
  fabItemBtn:
    'w-12 h-12 mb-3 rounded-full bg-secondary flex items-center justify-center text-lg text-secondary-foreground shadow-lg',
  fabItemLabel:
    'mr-3 bg-popover text-popover-foreground px-3 py-1 rounded-[--radius] text-sm font-medium shadow-lg whitespace-nowrap',

  // Modal and drawer using CSS variables
  modalBackdrop:
    'modal-backdrop absolute inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-20',
  modalContent:
    'modal-content bg-card rounded-[--radius] shadow-xl max-w-md w-full mx-4 p-6 relative border border-border',
  modalClose:
    'modal-close absolute top-4 right-4 text-muted-foreground hover:text-foreground w-8 h-8 flex items-center justify-center rounded-full hover:bg-accent transition-colors',
  drawer:
    'drawer absolute top-0 left-0 z-20 w-64 h-full bg-card border-r border-border shadow-lg transform -translate-x-full transition-transform duration-300 ease-in-out',
...
export function getButtonInlineStyles(
  ...
  const baseStyle =
    'border-radius: var(--radius); focus-ring-color: var(--ring);'
 ...
 */
export function getLayoutModifierInlineStyles(
