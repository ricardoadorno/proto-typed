 ...
 * @returns {LayoutModifiers} An object containing the parsed layout modifiers.
 */
export function parseLayoutModifiers(tokenImage: string): LayoutModifiers {
 ...
 */
export function parseNavigatorItem(itemText: string): AstNode {
  // Try 3-part pipe format: "- text | icon | destination"
  const threePartMatch = itemText.match(
    /^-\s+([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*(.+)$/
  )
  if (threePartMatch) {
    return {
