 ...
 */
export function buildLayoutElement(ctx: CstContext, visitor: CstVisitor) {
  const elements = []
  ...
  if (ctx.element && ctx.element.length > 0) {
    for (const el of ctx.element) {
      const elementAst = visitor.visit(el as CstNode)
      if (elementAst) {
 ...
 */
export function buildListElement(ctx: CstContext) {
  ...
  if (ctx.UnorderedListItem) {
    ctx.UnorderedListItem.forEach((item) => {
      const itemText = (item as IToken).image
 ...
 */
export function buildUnorderedListElement(ctx: CstContext) {
  const listText = (ctx.UnorderedListItem[0] as IToken).image
 ...
 */
export function buildNavigatorElement(ctx: CstContext) {
  const items: AstNode[] = []
 ...
 */
export function buildSeparatorElement(_ctx: CstContext) {
  return {
    type: 'Separator',
    id: '', // ID will be generated later
    props: {},
 ...
 * @returns {object} A 'Navigator' AST node.
 */
export function buildNavItemElement(ctx: CstContext) {
