  ...
  return (
    <div className="flex h-full w-full items-center justify-center rounded-3xl border border-[var(--border-muted)] bg-[var(--bg-raised)] p-6">
      {deviceType === 'browser-mockup with-url' ? (
        <BrowserMockup zoom={_zoom}>{children}</BrowserMockup>
      ) : (
        <IPhoneMockup zoom={_zoom}>{children}</IPhoneMockup>
      )}
    </div>
  )
}
...
function IPhoneMockup({
  ...
  return (
    <div
      className="relative mx-auto h-[812px] w-[375px] overflow-hidden rounded-[2.5rem] border border-[color:rgba(139,92,246,0.25)] bg-[var(--bg-main)] shadow-[0_0_0_6px_rgba(17,18,26,0.8),0_32px_64px_rgba(12,14,24,0.45)]"
      style={{
    ...
    >
      {/* Home indicator */}
      <div className="absolute bottom-2 left-1/2 h-1 w-[140px] -translate-x-1/2 transform rounded-sm bg-[color:rgba(169,175,191,0.4)] z-30"></div>

      {/* Notch */}
      <div className="absolute left-1/2 top-0 z-50 h-[30px] w-[56%] -translate-x-1/2 transform rounded-b-[1.25rem] bg-[var(--bg-main)]"></div>

      {/* Speaker */}
      <div className="absolute left-1/2 top-0 z-50 h-2 w-[15%] -translate-x-1/2 translate-y-1.5 transform rounded bg-[var(--bg-surface)] shadow-[inset_0_-1px_2px_rgba(255,255,255,0.08)]"></div>

      {/* Camera */}
      <div className="absolute left-[15%] top-0 z-50 h-3 w-3 translate-x-[180px] translate-y-1 transform rounded-full bg-[var(--bg-surface)] shadow-[inset_0_-1px_3px_rgba(255,255,255,0.08)]">
