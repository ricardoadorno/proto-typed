...
<DocsCodePreview title="Grid-responsive para coleções flexíveis">
```dsl
screen Default:
  container:
    grid-responsive:
...
<Tip title="Alias legado">
`grid-auto` continua disponível como alias de `grid-responsive` para manter snippets antigos válidos.
</Tip>
...
| --- | --- | --- | --- | --- |
| `variante` | enum (`grid-2`, `grid-3`, `grid-4`, `grid-responsive`) | `grid-3` | Define número de colunas ou ajuste automático. | Não |
