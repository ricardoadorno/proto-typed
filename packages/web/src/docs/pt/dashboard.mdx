...
<DocsCodePreview title="Estrutura completa do dashboard">
```dsl
...
screen Dashboard:
  header:
    # Painel de Operações
    row-end:
      @ghost[Atualizar]{refresh-ccw}(refresh)
      @ghost[Filtros]{filter}(FilterDrawer)
...
drawer FilterDrawer:
  container:
    # Filtros do painel
    ___[Intervalo][Selecione intervalo[7 dias | 14 dias | 30 dias]]
    ___[Equipe][Selecione equipe[Todos | Produto | Engenharia | Suporte]]
    row-end:
