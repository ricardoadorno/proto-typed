...
export default function PlaygroundPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-main)] text-[var(--fg-primary)] flex flex-col">
      <DocsHeader />
      <main className="mx-auto flex w-full max-w-full lg:max-w-7xl xl:max-w-[1400px] flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8 pb-16">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl sm:text-4xl font-semibold leading-tight text-[var(--fg-primary)]">
          Playground
        </h1>
        <p className="text-[var(--fg-secondary)] text-sm sm:text-base">
          Experimente a sintaxe DSL em tempo real. Use <code className="px-1.5 py-0.5 bg-[var(--bg-surface)] border border-[var(--border-muted)] rounded text-xs">@core</code> para links internos e <code className="px-1.5 py-0.5 bg-[var(--bg-surface)] border border-[var(--border-muted)] rounded text-xs">@web</code> para externos.
        </p>
      </div>
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-2 max-w-full">
        <Card className="flex h-full flex-col gap-4 border border-[var(--border-muted)] bg-[var(--bg-surface)] p-4 md:p-6 shadow-[0_24px_64px_rgba(14,16,24,0.38)]">
        <CardHeader className="space-y-1.5 p-0">
          <CardTitle className="text-lg md:text-xl text-[var(--fg-primary)]">Editor DSL</CardTitle>
          <CardDescription className="text-sm text-[var(--fg-secondary)]">
          Escreva componentes, telas e fluxos. Nova sintaxe v0.0.2 com @core e @web!
          </CardDescription>
        </CardHeader>
        <ExampleButtons
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-2 sm:gap-3">
          ...
          <Link target="_blank" href={("/docs")}>Ver docs</Link>
          </Button>

          <Button onClick={handleExport} size="sm" variant="ghost" className="gap-2 border border-[var(--border-muted)] text-[var(--fg-secondary)] hover:border-[var(--brand-400)] hover:text-[var(--accent)]">
        ...
        <Card className="flex h-full flex-col gap-4 border border-[var(--border-muted)] bg-[var(--bg-surface)] p-4 md:p-6 shadow-[0_24px_64px_rgba(14,16,24,0.38)]">
        <CardHeader className="space-y-1.5 p-0">
          <CardTitle className="text-lg md:text-xl text-[var(--fg-primary)]">Preview ao vivo</CardTitle>
          <CardDescription className="text-sm text-[var(--fg-secondary)]">
          Visualize e interaja com a interface em tempo real.
          </CardDescription>
        </CardHeader>
