...
## Exemplo básico
<DslPreview title="Seção textual completa">
```dsl
screen ReportIntro:
  container:
    # Relatório semanal
...
## Variantes
<DslPreview title="Notas e citações">
```dsl
card:
...
<DslPreview title="Metadados sutis">
```dsl
card:
  ## Status do projeto
...
<DslPreview title="Títulos hierárquicos">
```dsl
container:
  # Visão geral
  ## Métricas principais
...
<DslPreview title="Card com texto e ação">
```dsl
card:
  ## Próxima entrega
...
**Não faça — usar mais de um título principal por tela**
<DslPreview title="❌ Errado — Dois `#` na mesma tela">
```dsl
screen Summary:
  container:
    # Resumo
...
Faça — distribua hierarquia com subtítulos
<DslPreview title="✅ Correto — Uso de subtítulos">
```dsl
screen Summary:
  container:
    # Resumo
...
**Não faça — misturar ações dentro de citações**
<DslPreview title="❌ Errado — Citação com botão">
```dsl
card:
...
Faça — separe ação do bloco textual
<DslPreview title="✅ Correto — Citação e ação organizadas">
```dsl
card:
...
<DslPreview title="Texto guiando ação">
```dsl
card:
  ## Atualize o status
